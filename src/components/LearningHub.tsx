...
import { Code2, Zap, Cpu, ArrowRight, ChevronLeft, BookOpen, Shield, Gauge, FileCode, GitBranch, TestTube, Server, Database, Layers, CheckCircle2 } from 'lucide-react';
...
function LearnCard({ icon, title, description, points, isCompleted, onClick }: LearnCardProps) {
  return (
    <div 
      onClick={onClick}
      className={`p-8 border rounded-2xl transition-all duration-300 group cursor-pointer h-full flex flex-col hover:shadow-xl hover:-translate-y-1 relative overflow-hidden ${
        isCompleted ? 'bg-green-50/50 border-green-200 hover:bg-green-50' : 'bg-white/60 border-ink/20 hover:bg-white'
      }`}
    >
      {isCompleted && (
      ...
      <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform shrink-0 ${
        isCompleted ? 'bg-green-100 text-green-700' : 'bg-ink/5'
      }`}>
        {icon}
      </div>
      <h4 className="font-heading font-bold text-2xl mb-3">{title}</h4>
      <p className="text-sm text-ink/60 leading-relaxed mb-6 flex-grow">{description}</p>
      <ul className="space-y-2 mt-auto">
        {points.map((p, i) => (
          <li key={i} className="flex items-center gap-2 text-xs font-medium opacity-70">
            <div className={`w-1 h-1 rounded-full shrink-0 ${isCompleted ? 'bg-green-500' : 'bg-ink'}`} />
            {p}
          </li>
        ))}
      </ul>
...
export default function LearningHub({ selectedLessonId, onCompleteLesson, completedLessons = [], onResetLessonSelection, onSelectLesson }: LearningHubProps) {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'code': return <Code2 className="text-blue-600" />;
      case 'zap': return <Zap className="text-orange-500" />;
      case 'cpu': return <Cpu className="text-purple-600" />;
      case 'arrow': return <ArrowRight className="text-green-600" />;
      case 'shield': return <Shield className="text-red-500" />;
      case 'gauge': return <Gauge className="text-teal-600" />;
      case 'file-code': return <FileCode className="text-blue-500" />;
      case 'git-branch': return <GitBranch className="text-orange-600" />;
      case 'test-tube': return <TestTube className="text-green-500" />;
      case 'server': return <Server className="text-indigo-500" />;
      case 'database': return <Database className="text-emerald-600" />;
      case 'layers': return <Layers className="text-pink-500" />;
  ...
  const selectedChapter = selectedLesson?.chapters?.find(c => c.id === selectedChapterId);
        ...
        {!selectedLesson ? (
          ...
          >
            <div className="text-center space-y-4">
              ...
              <p className="text-ink/60 max-w-xl mx-auto">Chọn một chủ đề để bắt đầu học từ cơ bản đến nâng cao.</p>
            </div>
                ...
                <div className="bg-white/80 border border-ink/20 rounded-2xl p-6 shadow-sm sticky top-6">
                  <div className="flex items-center gap-4 mb-6 pb-6 border-b border-ink/10">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-ink/60 mt-1">{selectedLesson.chapters?.length || 0} bài học</p>
                    </div>
                  </div>
                    ...
                    {selectedLesson.chapters?.map((chapter, index) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <span className="text-xs font-mono opacity-60 mb-1 block">Bài {index + 1}</span>
                  ...
                  {selectedLesson && (
                    <div className="mt-6 pt-6 border-t border-ink/10">
                      <button
                        onClick={() => onCompleteLesson && onCompleteLesson(selectedLesson.id)}
                      ...
                      >
                        {completedLessons.includes(selectedLesson.id) ? 'Đã hoàn thành (Hoàn tác)' : 'Đánh dấu hoàn thành'}
                      </button>
                    </div>
                  )}
                </div>
                        ...
                        components={{
                          code({node, inline, className, children, ...props}: any) {
                            ...
                            return !inline && match ? (
                              ...
                              <code {...props} className="bg-ink/10 px-1.5 py-0.5 rounded-md font-mono text-sm text-pink-600">
                                {children}
                              </code>
                            )
                          }
